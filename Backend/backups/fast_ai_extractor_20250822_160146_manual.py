...
class FastAIExtractor:
    ...
    def _detect_real_estate_site_from_content(self, content_text: str) -> bool:
        ...
        try:
            ...
            # Indicatori universali per siti immobiliari (multilingua)
            real_estate_indicators = [
                # Italiano
                'mq', 'metri quadri', 'bagno', 'bagni', 'camera', 'camere',
                'piano', 'vendita', 'affitto', 'bilocale', 'trilocale', 'quadrilocale',
                'appartamento', 'villa', 'casa', 'immobile', 'proprietà', 'terreno',
                
                # Inglese
                'sqm', 'square meters', 'bathroom', 'bathrooms', 'bedroom', 'bedrooms',
                'floor', 'for sale', 'for rent', 'studio', 'apartment', 'house',
                'property', 'real estate', 'land', 'acres', 'sq ft',
                
                # Francese
                'm²', 'mètres carrés', 'salle de bain', 'chambre', 'étage',
                'à vendre', 'à louer', 'appartement', 'maison', 'propriété',
                
                # Tedesco
                'qm', 'quadratmeter', 'bad', 'bäder', 'zimmer', 'stockwerk',
                'zu verkaufen', 'zu vermieten', 'wohnung', 'haus', 'grundstück',
                
                # Spagnolo
                'm²', 'metros cuadrados', 'baño', 'baños', 'habitación', 'plantas',
                'en venta', 'en alquiler', 'apartamento', 'casa', 'propiedad'
            ]
            
            # Conta quanti indicatori immobiliari sono presenti
            real_estate_score = sum(1 for indicator in real_estate_indicators if indicator in content_lower)
            
            # Se ci sono molti indicatori immobiliari, è probabilmente un sito immobiliare
            return real_estate_score >= 3
...
INSTRUCTIONS:
1. Look for REAL products with REAL prices (€XX.XX format)
2. Focus on actual products, NOT navigation categories or menu items
3. Extract products that have:
   - A specific product name (not generic categories like "Computer", "Modem Router")
   - A real price (not "0,00€" unless it's actually free)
   - Product details or specifications

4. IGNORE these items (they are NOT products):
   - Navigation menu items
   - Category names
   - Generic terms like "Computer", "Modem Router", "Binocoli"
   - Items with price "0,00€" (unless they're actually free products)

5. For EACH product, extract ALL available characteristics:
   - name: Specific product name with brand/model
   - price: Real price in € format
   - brand: Brand name if mentioned
   - model: Model number if mentioned
   - weight: Weight if mentioned (g, kg, etc.)
   ...
   - specs: Technical specifications if available
...
Return this JSON format with ONLY REAL PRODUCTS and ALL their characteristics:
{{"products": [{{"name": "Specific Product Name with Brand/Model", "price": "€XX.XX", "brand": "Brand Name", "model": "Model Number", "weight": "Weight", "price_per_kg": "€X.XX/Kg", "rating": "X.X", "description": "Product description", "category": "Category", "availability": "In Stock", "warranty": "Warranty info", "specs": "Technical specs"}}]}}

CRITICAL: Only extract actual products with real names and prices. Skip navigation items and categories. Include ALL available characteristics for each product."""
            ...
            for selector in candidate_selectors:
                try:
                    ...
                    if valid_elements >= min_valid_elements and valid_elements >= len(elements[:5]) * min_ratio:
                        ...
                        print(f"✅ Selettore appreso: {selector} - {len(elements)} elementi, {valid_elements} validi (immobili: {is_real_estate})")
